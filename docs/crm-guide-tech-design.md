...
- 当前已完成第九批清理：
  - 运行时默认品牌、导购、门店名称改为中性值，不再把 demo 身份当作生产默认配置
  - `complete_task` 与 bootstrap 预览客户查询兼容“未配置门店名称”的运行环境，避免空身份配置导致动作链路失效
- 当前已完成第十批清理：
  - 空状态与桌面侧栏的详情预览入口统一改成中性文案
  - 当 bootstrap 没有可用 `preview_customer_id` 时，详情预览入口进入禁用态，避免无反馈点击
- 当前已完成第十一步清理：
  - bootstrap 的 `quick_prompts` 改为“配置优先、数据兜底”
  - 当环境未显式配置快捷问题时，系统会根据当前预览客户、客户偏好和待办任务生成更贴近真实工作流的首屏问题
- 当前已完成第十二批清理：
  - 新增协议同步测试，直接读取后端 `crm_service.py` 中的组件与动作定义
  - 前端 `rendererRegistry` 与 `registeredActionTypes` 会在测试中对齐后端输出，减少后端新增协议但前端漏注册的风险
