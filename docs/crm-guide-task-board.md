...
- [x] 目标：建立 `main`、`develop`、`test` 分支和远端仓库映射
- 依赖：任务 `0.1.1`
...
- [x] 目标：对当前系统做一轮基线测试，形成 `v0.1.0` 发布记录
- 依赖：任务 `0.1.2`
...
- [x] 目标：定义稳定型问题、生成型问题、重复提问策略和换批条件
- 依赖：`v0.1.0`
...
- 新增 `repeat_query_mode`：`fresh / preserve / diversify`
- 新增 `stability_mode`，把“实体稳定”和“文本可变”写进响应 meta
- 明确同会话重复提问默认保持，显式“换一批”才换候选
- 为重复提问策略补齐后端测试
...
- 已完成第一批迁移：默认返回数量、最大返回数量、客户总览样本数、维护场景商品数、快捷提示
- 已完成第二批迁移：可见商品/任务数量与 `focus_scope`、`session_snapshot.last_entity_ids` 对齐，不再残留内部固定 `4` 条截断
- 已完成第三批迁移：工作流说明条数和客户记忆摘要条数已迁入配置层，不再散落在服务实现里
- 已完成第四批迁移：商品标签预览上限和 guardrail 默认数量也已配置化，运行时代码里不再残留核心业务字面量 `4`
- 已完成第五批迁移：前端头部不再伪造默认品牌、顾问、门店展示文案，缺字段时改为中性占位
- 已完成第六批迁移：聊天入口限流阈值和窗口时长已迁入配置层，并补了 API 集成回归
- 已完成第七批迁移：mutation 限流阈值和窗口时长已迁入配置层，任务与记忆操作共享同一治理规则
- 已完成第八批修复：前端在触发限流时会返回可理解提示，不再一律展示通用失败文案
- 已完成第九批修复：空状态快捷入口直接渲染配置下发的 prompt，不再映射为固定 demo 标题
- 已完成第十批修复：空状态摘要卡和桌面侧栏改为用途说明，不再伪造具体客户与结果数量
- 已完成第十一步修复：空状态“查看客户详情”入口改为消费 bootstrap 下发的真实 `preview_customer_id`，不再写死 `C001`
- 已完成第十二批修复：运行时默认品牌、导购、门店名称改为中性值，避免未配置环境直接暴露 demo 身份
- 待继续迁移：更多结果摘要、部分截断切片和其他表现层裁剪
...
- 已移除客户概览和会话快照相关的固定 `4` 条断言
- E2E 已改为验证“样本数量正确展示”，不再把某个 demo 数字当成产品契约
- 当前端缺少 `sample_limit` 时，展示文案不再凭空回退到 `4`
- 已补任务完成后的缓存失效回归，防止重新提问时回到旧任务结果
...
- [x] `test` 分支本地 smoke：`/health`、`/api/crm/bootstrap`、导购域聊天请求通过
- [x] `main` 分支本地 smoke：`/health`、`/crm`、拒答链路通过
- [ ] 独立部署环境的真实线上回归
