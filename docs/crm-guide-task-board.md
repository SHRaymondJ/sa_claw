...
- 已完成第十三批修复：空状态与桌面侧栏的详情入口改成中性文案，并在没有预览客户时禁用，避免用户点击无效入口
- 已完成第十四批修复：bootstrap 快捷问题改为“优先使用配置，缺省时按当前客户与任务动态生成”，空状态不再依赖固定模板问题
- 已完成第十五批修复：新增协议同步测试，自动校验后端 `component_type` 与 `action_type` 是否都已在前端 registry 注册
